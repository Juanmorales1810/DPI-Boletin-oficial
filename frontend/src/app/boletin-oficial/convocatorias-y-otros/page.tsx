...
export default function LegislacionAvisosOficiales() {
    ...
    const { data, isLoading } = useSWR(`http://localhost:8000/buscador-publicaciones?page=${page}${tipoBoletin.convocatorias ? "&tipoPublicacion=Convocatorias" : ""}${tipoBoletin.otros && ""}${searchTerm !== "" ? `&nombre=${searchTerm}` : ""}&fechaInicio=${isActive ? dateBuscar : ""}&pageSize=4`, fetcher, {
            ...
            <aside className="w-1/4 p-4 bg-gray-100 hidden flex-col justify-start items-center md:flex">
                <div className="mt-12">
                    ...
                    <ul>
                        ...
                        <li className="mb-2 text-naranjaPrincipal"><Link href="#">Convocatorias y otros</Link></li>
                    </ul>
                </div>
            </aside>
            ...
            <aside className="w-full p-4 bg-gray-100 mt-2 md:w-1/4 md:pt-14 md:pl-20">
                ...
                <div className="flex justify-center items-center gap-4 md:flex-col md:items-start md:gap-0">
                    <div className="flex items-center space-x-2 py-2">
                        <Checkbox
                            id="convocatorias"
                            checked={tipoBoletin.convocatorias}
                            onCheckedChange={() => setTipoBoletin({ ...tipoBoletin, convocatorias: !tipoBoletin.convocatorias })}
                        />
                        <label
                    ...
                    <div className="flex items-center space-x-2 py-2">
                        <Checkbox
                            id="otros"
                            checked={tipoBoletin.otros}
                            onCheckedChange={() => setTipoBoletin({ ...tipoBoletin, otros: !tipoBoletin.otros })}
                        />
                        <label
                            htmlFor="otros"
                            className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 select-none"
